...
const PRODUCTS = [
  { id:1, name:"Polo Signature YkoniC", brand:"YkoniC", cat:"Polos",
    price:18500, badge:"Bestseller",
    grad:"from-stone-100 to-stone-200", emoji:"👕",
    sizes:["S","M","L","XL","XXL"],
    desc:"Polo signature en piqué coton premium 220g. Col côtelé renforcé, broderie couronne YkoniC sur poitrine gauche. Coupe ajustée moderne." },
  { id:2, name:"Ensemble T-Shirt & Short", brand:"YkoniC", cat:"Ensembles",
    price:27500, badge:"New",
    grad:"from-zinc-100 to-slate-200", emoji:"👕",
    sizes:["S","M","L","XL"],
    desc:"Set coordonné T-shirt oversize + short assorti. Jersey coton 220g, logo YkoniC imprimé, taille élastique." },
  { id:3, name:"Survêtement Elite", brand:"YkoniC", cat:"Survêtements",
    price:42000, badge:"New",
    grad:"from-gray-100 to-gray-200", emoji:"🥋",
    sizes:["S","M","L","XL","XXL"],
    desc:"2 pièces veste zippée + pantalon. Molleton brossé 320g, bandes contrastées, poches zippées latérales. Logo brodé." },
  { id:4, name:"Jogging YkoniC Sport", brand:"YkoniC", cat:"Joggings",
    price:19500,
    grad:"from-neutral-100 to-neutral-200", emoji:"👖",
    sizes:["S","M","L","XL"],
    desc:"Coupe slim, taille élastique avec cordon, chevilles resserrées. Molleton léger respirant. Logo imprimé cuisse." },
  { id:5, name:"Casquette Crown", brand:"YkoniC", cat:"Casquettes",
    price:8500, badge:"Bestseller",
    grad:"from-amber-50 to-yellow-100", emoji:"🧢",
    sizes:["Unique"],
    desc:"6 panneaux, broderie couronne YkoniC en relief sur la face, fermeture snapback noire ajustable." },
  { id:6, name:"Jean Slim Premium", brand:"Partenaire", cat:"Jeans",
    price:24500,
    grad:"from-blue-100 to-indigo-100", emoji:"👖",
    sizes:["28","30","32","34","36"],
    desc:"Denim stretch 98/2, coupe slim 5 poches, délavage moyen. Résistant au quotidien." },
  { id:7, name:"Sneakers Cuir Blanc", brand:"Partenaire", cat:"Chaussures",
    price:38000, badge:"New",
    grad:"from-slate-50 to-gray-100", emoji:"👟",
    sizes:["40","41","42","43","44","45"],
    desc:"Cuir véritable blanc, semelle gomme légère, intérieur rembourré. Élégant et polyvalent." },
  { id:8, name:"Sac Voyage Minimaliste", brand:"Partenaire", cat:"Accessoires",
    price:47000, badge:"New",
    grad:"from-stone-200 to-amber-100", emoji:"🎒",
    sizes:["Unique"],
    desc:"40L nylon ripstop imperméable. Compartiment laptop 15\", sangles cachées. Design urbain minimaliste." },
]
  ...
  return (
    <div onClick={() => onView(p)}
      ...
      <div className="relative aspect-[4/5] overflow-hidden bg-stone-50">
        <div className={`w-full h-full bg-gradient-to-br ${p.grad} flex items-center justify-center transition-transform duration-500 group-hover:scale-105`}>
          <span className="text-7xl opacity-30 select-none">{p.emoji}</span>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-14 mb-20">
          ...
          <div className="space-y-4">
            <div className={`aspect-[4/5] rounded-3xl bg-gradient-to-br ${p.grad} flex items-center justify-center relative overflow-hidden border border-amber-100`}>
              <span className="text-[130px] opacity-25 select-none">{p.emoji}</span>
              <div className="absolute inset-0 bg-gradient-to-tr from-amber-50/10 to-transparent" />
              {/* Logo watermark */}
            ...
            <div className="grid grid-cols-4 gap-3">
              {[p.grad,"from-gray-100 to-gray-200","from-stone-100 to-stone-200","from-zinc-100 to-zinc-200"].map((g,i) => (
                <div key={i} className={`aspect-square rounded-xl bg-gradient-to-br ${g} flex items-center justify-center border ${i===0?"border-amber-400 ring-2 ring-amber-300/50":"border-stone-100 opacity-50 hover:opacity-80 cursor-pointer"} transition-all`}>
                  <span className="text-2xl opacity-40">{p.emoji}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          {items.map(item => (
            <div key={item.id} className="flex gap-5 p-4 bg-white border border-stone-100 rounded-2xl shadow-sm hover:border-amber-100 transition-all">
              <div className={`w-24 h-28 rounded-xl bg-gradient-to-br ${item.grad} flex items-center justify-center flex-shrink-0 border border-amber-50`}>
